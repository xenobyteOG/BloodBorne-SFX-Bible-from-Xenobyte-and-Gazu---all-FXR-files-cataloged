...
One thing that does usually hold true is boss SFXs are in the 600000 range. For instance if you want to move a boss from one map to another then look in the original map's SFX DCC archive in the f000600000-f000700000 range to find the boss special effects. See if you can match up the boss SFX with our crazy descriptions in that range on their original map. Then take them and then add them to the map SFX DCX you are adding the boss to. That way you have the full SFX of the boss on the new map. 

Here is a small note - we found that the crashy/broken SFX were, alot of times, due to broken TEXTURES not sfx. The FXR calles a texture, like say for the Celestial Shell magic weapon buff calls texture 0003.tpf. Well, that texture is the issue, you take it out and the sfx works. That, hopefully, helps people somehow knowing that. 
